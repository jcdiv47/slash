...
const ShortcutFrame = ({ shortcut }: Props) => {
  return (
    <div className="w-full h-full flex flex-col justify-center items-center p-8">
      <Link
        className="w-72 max-w-full border border-border bg-card text-card-foreground p-6 pb-4 rounded-md hover:opacity-80"
        to={`/s/${shortcut.name}`}
        target="_blank"
      >
        <div className="w-12 h-12 flex justify-center items-center overflow-clip rounded-md shrink-0">
          <LinkFavicon url={shortcut.link} />
        </div>
        <p className="text-lg font-medium leading-8 mt-2 truncate">{shortcut.title || shortcut.name}</p>
        <p className="shortcut-name text-sm text-muted-foreground truncate">s/{shortcut.name}</p>
